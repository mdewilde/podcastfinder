...
public class ITunesFeedGeneratorAction implements PodcastFinderAction {

	@Override
	public void run() {
		FeedGeneratorFinder feedGeneratorFinder = new FeedGeneratorFinder();
		try (FeedIdBatchingStore store = new FeedIdBatchingStore(1000)) {
			feedGeneratorFinder.search(store::add);
		}
	}
