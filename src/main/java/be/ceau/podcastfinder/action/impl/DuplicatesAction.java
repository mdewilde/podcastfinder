...
public class DuplicatesAction implements PodcastFinderAction {

	@Override
	public void run() {
		try (PodcastFinderStore store = new PodcastFinderStore()) {
			Set<Duplicates> set = store.getDuplicates();
			
			set.stream().map(Duplicates::toConsole).forEach(System.out::println);
			set.stream()
				.flatMap(d -> d.getRemovableIds().stream())
