...
import be.ceau.podcastparser.models.core.RFC639;

public class CleanAction implements PodcastFinderAction {
	...
	public void run() {
		try (PodcastFinderStore store = new PodcastFinderStore()) {
			...
			store.getLanguages()
					...
					.filter(l -> {
						try {
							return RFC639.valueOf(l) == null;
						} catch (IllegalArgumentException e) {
							return true;
						}
					})
					.peek(System.out::println)
					.forEach(store::deleteLanguage);
		}
	}
